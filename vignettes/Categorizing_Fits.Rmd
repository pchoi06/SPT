...
We need to fit the sigmoidal and double-sigmoidal models to this dataset, using `multipleFitFunction_h0()`. This requires first normalizing the data:
```{r}
...
We also need to perform the additional parameter calculations, as these are required by the `Categorize_h0()` function we use below.

```{r linear sigmoidal amd double-sigmoidal fits to double-sigmoidal data}
# Calculate additional parameters
sigmoidalModel <- parameterCalculation_h0(sigmoidalModel)
...
f1 <- figureModelCurves_h0(dataInput = normalizedInput,
                        sigmoidalFitVector = sigmoidalModel,
                        showParameterRelatedLines = TRUE)

f2 <- figureModelCurves_h0(dataInput = normalizedInput,
...
Clearly the sigmoidal fit is not appropriate but the double-sigmoidal one is. Next we demonstrate how to arrive at this conclusion computationally, using the function `Categorize_h0()`. It takes as input the two fitted models as well as a number of parameters that are used in the decision process (explained below under "The decision process").


```{r}
# now we can categorize the fits
decisionProcess <- Categorize_h0(threshold_minimum_for_intensity_maximum = 0.3,
...
* Pre-test 4a: Have the additional parameters for sigmoidal fit been calculated by `parameterCalculation_h0()`?
* Pre-test 4b: Have the additional parameters for double-sigmoidal fit been calculated by `parameterCalculation_h0()`?
